...
padded_angles = np.full((n_events, max_choices), np.nan, dtype=np.float64) 
...
with pm.Model(coords=coords) as hierarchical_pass_choice_model:
    ...
    mu_k_direction = pm.Normal("mu_k_direction", mu=1.0, sigma=1.0) 
    sigma_k_direction = pm.HalfNormal("sigma_k_direction", sigma=1.0) 
    sigma_k_direction = sigma_k_direction 
...
def calculate_accuracy_hierarchical_fixed_denominator(eval_data, idata, player_id_mapping, fixed_denominators, description=""):
    ...
    try:
        ...
        # --- CHANGE HERE ---
        mean_k_direction_player = idata.posterior["k_direction"].mean(dim=("chain", "draw")).values 
    except Exception as e:
    ...
    for event_row_data in eval_data:
        ...
        # --- CHANGE HERE ---
        k_direction_val = mean_k_direction_player[player_id]
...
def evaluate_posterior_accuracy_per_player(eval_data, idata,description=""):
    ...
    for player_id, events in player_groups.items():
        ...
        #df_outputs.to_csv(f"player_{player_id}_posterior_best_sample_simple.csv", index=False)

        # Save full posterior parameter values and accuracies
        df_params = pd.DataFrame(param_records)
        #df_params.to_csv(f"player_{player_id}_posterior_parameters_simple.csv", index=False)

        df_accs = df_params[["sample_position", "accuracy"]]
        #df_accs.to_csv(f"player_{player_id}_posterior_accuracies_simple.csv", index=False)
