...
import pandas as pd
from shapely.geometry import Point, Polygon
import math # Ensure math is imported for atan2 and degrees/radians

# Assume triangle_density_weighted is only called when height > 15
def triangle_density_weighted_dynamic(rel_y, height):
    """
    Calculates a density based on a relative y-coordinate (rel_y)
    and the pass height. Peaks are located at fractional distances
    along the height from the ball's y-coordinate.

    Args:
        rel_y (float): The player's y-coordinate relative to the ball's y-coordinate (player_y - ball_y).
        height (float): The distance from the ball to the target.

    Returns:
        float: The density weight.
    """
    # Define the fractional distances along the height for the peaks
    # These correspond to 0.15 and 0.85 in the original scaled space (local_y)
    peak_fraction_1 = 0.15
    peak_fraction_2 = 0.85

    # Calculate the y-coordinates of the peaks relative to the ball's y
    peak_y_1 = peak_fraction_1 * height
    peak_y_2 = peak_fraction_2 * height

    # Use np.exp and power for numerical stability/vectorization potential
    # The factor '10' controls the sharpness of the peaks. Keep it consistent for now.
    density = np.exp(-((rel_y - peak_y_1) ** 2) * 10 / (height/15)) + \
              np.exp(-((rel_y - peak_y_2) ** 2) * 10 / (height/15))
              
    # Optional: Normalize density or ensure it's not excessively large/small
    # For now, let's return the raw sum of Gaussians.

    return density


# --- Weighted Overlap (Modified calculate_overlap_area) ---
def calculate_weighted_overlap_area_dynamic(target_x, target_y, x_ball, y_ball, away_players, angle_degrees=28.41389086074131, circle_radius=2):
    """
    Calculates the weighted overlap area of away players (as circles)
    with a triangle defined by the ball and a target point, incorporating
    density weighting based on the player's relative y-position when height > 15.

    This version uses the geometry calculation style of the original
    calculate_overlap_area but applies the weighting logic from the
    calculate_weighted_overlap_area function, with a density function
    whose peaks are based on the triangle's height.

    Args:
        target_x, target_y (float): Coordinates of the target point.
        x_ball, y_ball (float): Coordinates of the ball.
        away_players (DataFrame): DataFrame with 'x' and 'y' columns for player positions.
        angle_degrees (float): The full angle of the cone/triangle in degrees.
        circle_radius (float): The radius of the circle representing each player.

    Returns:
        float: The total weighted overlap area.
    """
    dx, dy = target_x - x_ball, target_y - y_ball
    height = np.hypot(dx, dy)

    # Handle cases where target is at the ball position
    if height < 1e-6:
        return 0.0
    ...
    # Handle edge case where half_angle is 90 degrees (cosine is zero)
    if abs(np.cos(half_angle_radians)) < 1e-9:
        return 0.0

    base_width = 2 * height * np.tan(half_angle_radians)

    # Calculate the two points on the target line forming the base of the triangle
    ...
    # Define the triangle using the ball position and the two base points
    points = [(left_x, left_y), (right_x, right_y), (x_ball, y_ball)]

    try:
        triangle = Polygon(points)
        # Check for invalid polygons (e.g., collinear points) or zero area
        if not triangle.is_valid or triangle.area < 1e-6:
            return 0.0
    except Exception:
        # Catch potential errors during Polygon creation (e.g., invalid geometry)
        return 0.0

    # --- Apply Weighted Overlap Logic ---
    total_weighted_overlap_area = 0

    # Iterate through away players to calculate weighted overlap
    for away_x, away_y in zip(away_players["x"], away_players["y"]):
        # Check for NaN values before processing the player
        if pd.notna(away_x) and pd.notna(away_y):
            player_point = Point(away_x, away_y).buffer(circle_radius)

            # Find the intersection area between the player's circle and the triangle
            intersection = triangle.intersection(player_point)

            # If there is an overlap
            if not intersection.is_empty:
                intersection_area = intersection.area

                # Calculate player's y-coordinate relative to the ball's y-coordinate
                rel_y = away_y - y_ball

                # Apply weighting based on the height condition
                if height > 15:
                    # Calculate the density for this player based on their relative y AND the pass height
                    density = triangle_density_weighted_dynamic(rel_y, height)
                    # Add the weighted intersection area to the total
                    total_weighted_overlap_area += intersection_area * density
                else:
                    # If height is 15 or less, add the raw intersection area (no weighting)
                    total_weighted_overlap_area += intersection_area

    return total_weighted_overlap_area
...
def preprocess_annotations_for_hierarchical_model(annotations_df, gps_df):
    ...
    for idx, ann_row in tqdm(annotations_df.iterrows(), total=len(annotations_df), desc="Processing combined annotations"):
        ...
        for i, (t_num, t_row) in enumerate(targets_df.iterrows()):
            ...
            overlap = calculate_weighted_overlap_area_dynamic(t_x, t_y, x_ball, y_ball, opponents_df)
...
with pm.Model(coords=coords) as hierarchical_pass_choice_model:
    ...
    mu_k_direction = pm.Normal("mu_k_direction", mu=1.0, sigma=1.0) # Prior for mean sensitivity
...
def evaluate_posterior_accuracy_per_player(eval_data, idata,description=""):
    ...
    for player_id, events in player_groups.items():
      ...
      #  df_outputs.to_csv(f"/Users/annadaugaard/Desktop/VFF/weighted_model_result_csv/player_{player_id}_posterior_best_sample_weighted.csv", index=False)

        # Save full posterior parameter values and accuracies
        df_params = pd.DataFrame(param_records)
      #  df_params.to_csv(f"/Users/annadaugaard/Desktop/VFF/weighted_model_result_csv/player_{player_id}_posterior_parameters_weighted.csv", index=False)

        df_accs = df_params[["sample_position", "accuracy"]]
       # df_accs.to_csv(f"/Users/annadaugaard/Desktop/VFF/weighted_model_result_csv/player_{player_id}_posterior_accuracies_weighted.csv", index=False)
...
# --- Posterior Predictive Accuracy Evaluation ---
    ...
    evaluate_posterior_accuracy_per_player(model_input_data, idata, description="Combined TRAINING")
