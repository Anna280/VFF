...
def evaluate_posterior_accuracy_per_player(eval_data, idata,description=""):
    ...
    for player_id, events in player_groups.items():
      ...
      #  df_outputs.to_csv(f"../weighted_model_result_csv/player_{player_id}_posterior_best_sample_weighted.csv", index=False)

        # Save full posterior parameter values and accuracies
        df_params = pd.DataFrame(param_records)
      #  df_params.to_csv(f"../weighted_model_result_csv/player_{player_id}_posterior_parameters_weighted.csv", index=False)

        df_accs = df_params[["sample_position", "accuracy"]]
       # df_accs.to_csv(f"../weighted_model_result_csv/player_{player_id}_posterior_accuracies_weighted.csv", index=False)
